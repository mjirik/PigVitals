...
def process_video(video_name):
    ...
    if response.status_code == 200:
        data = response.json()
        return redirect(url_for('show_results', video_name=video_name, video_length=data['video_length'], number_of_frames=data['number_of_frames']))
    else:
...
def show_results():
    ...
    return render_template('results.html', video_name=video_name, video_length=video_length, number_of_frames=number_of_frames)
